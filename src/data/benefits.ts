import type { BenefitItem } from "@/types/site";

export const benefits: BenefitItem[] = [
  {
    title: "Acompañamiento experto",
    description:
      "Contás con un equipo que te guía durante la planificación, coordinación y seguimiento de la operación internacional."
  },
  {
    title: "Mayor previsibilidad",
    description:
      "Analizamos alternativas antes de avanzar para que tomes decisiones con más contexto sobre tiempos, alcance y condiciones operativas."
  },
  {
    title: "Menos incertidumbre",
    description:
      "Ordenamos la información clave de la importación para reducir dudas frecuentes y anticipar pasos necesarios."
  },
  {
    title: "Trazabilidad",
    description:
      "Mantenemos seguimiento claro de la operación para que sepas en qué instancia está la carga y qué sigue."
  },
  {
    title: "Red logística nacional",
    description:
      "Integramos la operación internacional con la capacidad de distribución nacional de Mercure Global en Argentina."
  },
  {
    title: "Comunicación clara",
    description:
      "Priorizamos explicaciones simples, avances concretos y contacto ordenado durante cada etapa del proceso."
  },
  {
    title: "Soluciones según urgencia, volumen y presupuesto",
    description:
      "Coordinamos alternativas logísticas acordes a las necesidades reales de tu operación, sin forzar un único camino."
  }
];
